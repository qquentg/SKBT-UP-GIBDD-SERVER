...
Если Очевидец забанен, backend не удаляет его старые сообщения.
Но новые сообщения от этого Очевидца во время активного бана не создаются.

Для этих запросов backend вернет `403 Forbidden`:
...
Если в БД остались старые сообщения, созданные во время периода бана до этой проверки, они остаются скрытыми для `INSPECTOR` и `ADMIN`.
...
Если Очевидец заблокирован, новые сообщения от него не создаются и событие `message_created` по ним не отправляется.
...
Если чат Очевидца сейчас заблокирован, новая live-трансляция не стартует. Для старых live-трансляций, созданных до бана, события остаются доступными по общим правилам видимости чата.
